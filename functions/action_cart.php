...
class action_cart extends library{
  ...
  /*-------------------------------------------------*/
  public function addCart($total_price, $session_id, $name_buyer, $email_buyer, $phone_buyer, $address_buyer, $note_buyer, $type, $user_id){

    global $conn_vn;
    // $date_cart = $this->sw_get_current_weekday();
    $date_cart = date('Y-m-d');
    $sql = "INSERT into cart (session_id, total_price, date_cart, name_buyer, mail_buyer, phone_buyer, address_buyer, note_buyer, type, id_account) values ('".$session_id."',"."'".$total_price."',"."'".$date_cart."',"."'".$name_buyer."',"."'".$email_buyer."',"."'".$phone_buyer."',"."'".$address_buyer."',"."'".$note_buyer."',"."'".$type."',"."'".$user_id."')";
    $result = mysqli_query($conn_vn,$sql);  
  ...
  public function updateCartTable($total_price,$id_cart){
    ...
    $date_cart = $this->sw_get_current_weekday();
    $sql = "UPDATE cart SET total_price = '".$total_price ."' , discount = $money, percent = $percent, code_sale = '$code_sale' WHERE id_cart = $id_cart";
    $result = mysqli_query($conn_vn,$sql);  
    return 1;
  }
  /*-------------------------------------------------*/
  ...
  public function payment1 () {
      ...
      $session_id = session_id();    
      $id_cart = $this->addCart(0, $session_id, $name_buyer, $email_buyer, $phone_buyer, $address_buyer, $note_buyer, $type, $user_id);
      foreach ($rows as $row) {
          $this->addItemCart($row->id_product, $row->count, $row->price, $session_id, $row->color, $row->size, $row->id_color, $row->id_size, $id_cart);
          $totalPrice += $row->total;              
      }
