...
public function getProductList_byMultiLevel_orderProductId($idProductCat,$order_productId,$trang, $limit, $page){
    ...
    if (isset($_SESSION['price_min'])) {
        $whereClause .= " AND product_price_sale >= ".$_SESSION['price_min'];
    }
...
// }
    ...
    public function views ($id) {
        ...
        $views = $rows['product_views'];
        $views++;

        $sql = "UPDATE product SET product_views = $views WHERE product_id = $id";
        $result = mysqli_query($conn_vn, $sql);
    }
           
    public function da_xem ($id) {
        // unset($_SESSION['da_xem']);
        if (!isset($_SESSION['da_xem']) || empty($_SESSION['da_xem'])) {
            ...
            $count = count($_SESSION['da_xem']);
            if ($count > 4) {
                array_shift($_SESSION['da_xem']);
            }
