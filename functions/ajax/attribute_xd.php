<?php 
	...
	if (empty($_SESSION['attribute_xd'])) {
		$_SESSION['attribute_xd'][] = $value;
	} else {
		if (!in_array($value, $_SESSION['attribute_xd'])) {
			$check = 0;
			foreach ($_SESSION['attribute_xd'] as $k => $item) {
				$name_1 = $action->getDetail('thuoc_tinh_value', 'id', $item)['thuoc_tinh_id'];
				if ($name == $name_1) {
					$_SESSION['attribute_xd'][$k] = $value;
					$check = 1;
				}
			}
