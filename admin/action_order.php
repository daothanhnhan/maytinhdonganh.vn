...
//		$this->query($sql);
    ...
    public function updateOrder(){
        try {
            ...
            if (isset($_POST['id_cart']) && $_POST['id_cart'] != "") {
                ...
                $del = $this->del_code_sale($code_sale, $sale_state, $id_orderState);
                if ($del == true) {
                    $sale_state = 1;
                }
                /** List qty */
                ...
                // $state 
                $stmt->bind_param("sisssssdii", $note_cart, $id_orderState, $name_buyer, $phone_buyer, $address_buyer, $note_buyer, $mail_buyer, $cart_total_price, $sale_state, $id_cart);
                $stmt->execute();
    ...
    public function del_code_sale ($code, $state, $type) {
        global $conn_vn;
        if ($state == 0 && $type == 4) {
            $sql = "DELETE FROM code_sale WHERE code = '$code'";
            $results = mysqli_query($conn_vn, $sql);
            return true;
        } else {
            return false;
        }
